...
function badgeClasses(value: string) {
  if (value === "VERIFIED" || value === "PRIORITIZED") return "border-emerald-700 bg-emerald-950 text-emerald-700 dark:text-emerald-300"
  if (value === "REJECTED") return "border-red-800 bg-red-950 text-red-700 dark:text-red-300"
  if (value === "ESCALATED" || value === "COMPLEX") return "border-amber-700 bg-amber-950 text-amber-700 dark:text-amber-300"
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-5">
        {[
          ...
          <Card key={item.label} className="bg-background border-border text-foreground">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
        <Card className="border-border bg-background text-foreground">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Operational visibility into evidence automation, cohort focus, and pipeline quality.</CardDescription>
          </CardHeader>
        ...
        <Card className="border-border bg-background text-foreground">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Where the research program is concentrated right now.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Top disease areas</p>
              <div className="mt-3 space-y-2">
                {(analytics.topDiseaseAreas.length ? analytics.topDiseaseAreas : [{ label: "Unclassified", count: 0 }]).map((item) => (
                  <div key={item.label} className="flex items-center justify-between rounded-lg border border-border px-3 py-2 text-sm">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Hypothesis queue</p>
              <div className="mt-3 space-y-2">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card className="bg-background border-border text-foreground">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Capture ranked biomedical evidence with disease context and study design metadata.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={evidenceForm.studyType} onChange={(event) => setEvidenceForm((current) => ({ ...current, studyType: event.target.value }))}>
              ...
              <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={evidenceForm.evidenceDirection} onChange={(event) => setEvidenceForm((current) => ({ ...current, evidenceDirection: event.target.value }))}>
            ...
            <div className="space-y-2 pt-3">
              {evidence.slice(0, 5).map((record) => (
                <div key={record.id} className="rounded-xl border border-border p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{record.studyType} · {record.evidenceDirection.toLowerCase()} · score {record.evidenceScore}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{record.sourceLabel}{record.researchEntry?.externalId ? ` · ${record.researchEntry.externalId}` : ""} · provenance {prettyLabel(record.provenanceType)}</p>
                  <p className="mt-1 text-xs text-muted-foreground">review confidence {record.reviewConfidence}{record.reviewedByUser?.email ? ` · reviewer ${record.reviewedByUser.email}` : ""}{record.assignedReviewer?.email ? ` · assigned ${record.assignedReviewer.email}` : ""}</p>
                  {record.provenanceDetail ? <p className="mt-2 text-xs text-muted-foreground">{record.provenanceDetail}</p> : null}
                  {canReviewEvidence ? (
                    <div className="mt-3 grid gap-2 md:grid-cols-[180px_220px_1fr_auto]">
                      <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={reviewDrafts[record.id]?.reviewStatus ?? record.reviewStatus} onChange={(event) => updateReviewDraft(record.id, { reviewStatus: event.target.value })}>
                        {['AUTO_QUEUED', 'IN_REVIEW', 'VERIFIED', 'REJECTED', 'ESCALATED'].map((status) => <option key={status} value={status}>{status}</option>)}
                      </select>
                      <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={reviewDrafts[record.id]?.assignedReviewerId ?? record.assignedReviewer?.id ?? ""} onChange={(event) => updateReviewDraft(record.id, { assignedReviewerId: event.target.value })}>
                      ...
                      <Button variant="outline" className="border-border text-gray-200 hover:bg-gray-800" onClick={() => submitEvidenceReview(record.id)} disabled={saving === `review-${record.id}`}>{saving === `review-${record.id}` ? "Saving…" : "Update"}</Button>
                    </div>
                  ) : null}
                  {record.reviewEvents.length ? (
                    <div className="mt-3 rounded-xl border border-border p-3">
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Review history</p>
                      <div className="mt-3 space-y-2">
                        {record.reviewEvents.map((event) => (
                          <div key={event.id} className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                            <span className="rounded-full border border-border px-2 py-1 text-[11px] uppercase">{prettyLabel(event.eventType)}</span>
        ...
        <Card className="bg-background border-border text-foreground">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Capture disease and longevity hypotheses with intervention candidates and uncertainty-aware prioritization.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="rounded-xl border border-border p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Evidence selection</p>
              <div className="mt-3 grid gap-2">
                {evidence.slice(0, 8).map((record) => {
                  const selected = hypothesisForm.evidenceRecordIds.includes(record.id)
                  return (
                    <button key={record.id} type="button" className={`rounded-lg border px-3 py-3 text-left ${selected ? 'border-teal-600 bg-teal-950/40' : 'border-border bg-background'}`} onClick={() => toggleEvidenceSelection(record.id)}>
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-sm font-medium">{record.title}</span>
                        <span className={`rounded-full border px-2 py-1 text-[11px] uppercase tracking-[0.18em] ${badgeClasses(record.reviewStatus)}`}>{prettyLabel(record.reviewStatus)}</span>
                      </div>
                      <p className="mt-1 text-xs text-muted-foreground">score {record.evidenceScore} · {record.studyType}</p>
                    </button>
                  )
                })}
              </div>
            ...
            <div className="space-y-2 pt-3">
              {hypotheses.slice(0, 5).map((record) => (
                <div key={record.id} className="rounded-xl border border-border p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{record.status.toLowerCase()} · confidence {record.confidenceScore} · uncertainty {record.uncertaintyScore}</p>
                  <div className="mt-3 grid gap-2 md:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-border px-3 py-2 text-sm">
                      <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Evidence avg</p>
                    ...
                    <div className={`rounded-lg border px-3 py-2 text-sm ${
                      record.contraindicationScore >= 0.7
                        ? 'border-red-800 bg-red-950 text-red-700 dark:text-red-300'
                        : record.contraindicationScore >= 0.4
                        ? 'border-amber-700 bg-amber-950 text-amber-700 dark:text-amber-300'
                        : 'border-emerald-700 bg-emerald-950 text-emerald-700 dark:text-emerald-300'
                    }`}>
                      <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground flex items-center gap-1">
                        Contraindication
                  ...
                  {record.priorityChanges.length ? (
                    <div className="mt-4 rounded-xl border border-border p-3">
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Priority change graph</p>
                      <div className="mt-3 space-y-3">
                        {record.priorityChanges.map((change) => (
                          <div key={change.id} className="rounded-lg border border-border p-3">
                            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                              <span className="rounded-full border border-border px-2 py-1">{change.evidenceRecord.title}</span>
                              <span>→</span>
                              <span className="rounded-full border border-border px-2 py-1">{prettyLabel(change.evidenceReviewEvent.eventType)}</span>
                              <span>→</span>
                              <span className="rounded-full border border-border px-2 py-1">priority {change.previousPriorityScore} to {change.newPriorityScore}</span>
                            </div>
                            <p className="mt-2 text-xs text-muted-foreground">{change.previousStatus.toLowerCase()} → {change.newStatus.toLowerCase()} · delta {change.delta} · {new Date(change.createdAt).toLocaleString()}</p>
                            {change.rationale ? <p className="mt-2 text-sm text-muted-foreground">{change.rationale}</p> : null}
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <Card className="bg-background border-border text-foreground">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Define target cohorts for stratification, retrospective analysis, and trial readiness.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2 pt-3">
              {cohorts.slice(0, 5).map((record) => (
                <div key={record.id} className="rounded-xl border border-border p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{record.focusArea} · {record.cohortSize} members · {record._count?.trialMatches ?? 0} trial matches</p>
                  ...
                  {record.stratificationSummary ? <p className="mt-3 text-sm text-muted-foreground">{record.stratificationSummary}</p> : null}
                  <div className="mt-3 flex flex-wrap gap-2">
                    {record.stratificationAxes.map((axis) => <span key={axis} className="rounded-full border border-border px-2 py-1 text-xs text-muted-foreground">{axis}</span>)}
                  </div>
                  <div className="mt-4 rounded-xl border border-border p-3">
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Outcome backtest</p>
                    <div className="mt-3 grid gap-2 md:grid-cols-4">
                      {[
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">{record.calibration.backtestSummary}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="rounded-xl border border-border p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Top cohort focus areas</p>
              <div className="mt-3 space-y-2">
        ...
        <Card className="bg-background border-border text-foreground">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Track biomarker deltas over time so the platform compounds intervention-outcome intelligence. Visualize time series, compare protocols, and see confidence intervals.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
              ...
              <div className="mt-4">
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  {Array.from(new Set(outcomes.map(o => o.protocol?.name || "Unassigned"))).map(protocolName => (
                    <div key={protocolName} className="rounded border border-border p-2">
            ...
            <div className="space-y-2 pt-3">
              {outcomes.slice(0, 5).map((record) => (
                <div key={record.id} className="rounded-xl border border-border p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">baseline {record.baselineValue} → latest {record.latestValue} · delta {record.delta}</p>
                  <p className="mt-1 text-xs text-muted-foreground">protocol: {record.protocol?.name || "Unassigned"} · confidence: <span title="Confidence interval estimate">{record.confidenceScore}</span></p>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="bg-background border-border text-foreground xl:col-span-2">
          <CardHeader>
            <CardTitle>Trial matching workspace</CardTitle>
            <CardDescription className="text-muted-foreground">Persist trial candidates with rationale and cohort context for enrollment review and follow-through.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={trialForm.cohortId} onChange={(event) => setTrialForm((current) => ({ ...current, cohortId: event.target.value }))}>
            ...
            <div className="grid gap-3 pt-3 md:grid-cols-2">
              {trialMatches.slice(0, 6).map((record) => (
                <div key={record.id} className="rounded-xl border border-border p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{record.trialExternalId} · score {record.matchScore} · {record.status.toLowerCase()}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{record.cohort?.name ? `Cohort: ${record.cohort.name}` : "No cohort linked"}</p>
                  {canReviewTrialMatches ? (
                    <div className="mt-3 grid gap-2 md:grid-cols-[180px_220px_1fr_auto]">
                      <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={trialReviewDrafts[record.id]?.status ?? record.status} onChange={(event) => updateTrialReviewDraft(record.id, { status: event.target.value })}>
                        {["AUTO_QUEUED", "IN_REVIEW", "VERIFIED", "REJECTED", "ESCALATED"].map((status) => <option key={status} value={status}>{status}</option>)}
                      </select>
                      <select className="h-10 rounded-md border border-border bg-background px-3 text-sm text-foreground" value={trialReviewDrafts[record.id]?.assignedReviewerId ?? record.assignedReviewer?.id ?? ""} onChange={(event) => updateTrialReviewDraft(record.id, { assignedReviewerId: event.target.value })}>
                        <option value="">Unassigned reviewer</option>
                        {reviewers.map((reviewer) => <option key={reviewer.id} value={reviewer.id}>{reviewer.email ?? reviewer.name ?? reviewer.id} · {reviewer.role}</option>)}
                      </select>
                      ...
                      <Button variant="outline" className="border-border text-gray-200 hover:bg-gray-800" onClick={() => submitTrialMatchReview(record.id)} disabled={saving === `trial-review-${record.id}`}>{saving === `trial-review-${record.id}` ? "Saving…" : "Update"}</Button>
                    </div>
                  ) : null}
