...
function getCategoryChipClass(category: MarketplacePayoutRejectionCategory) {
  switch (category) {
    case "evidence_gap":
      return "border-sky-400/35 bg-sky-500/12 text-sky-800 dark:text-sky-100"
    case "compliance":
      return "border-violet-400/35 bg-violet-500/12 text-violet-800 dark:text-violet-100"
    case "milestone_scope":
      return "border-amber-400/35 bg-amber-500/12 text-amber-800 dark:text-amber-100"
    case "documentation":
...
function getSeverityBadgeClass(severity: MarketplacePayoutBlockerSeverity) {
  switch (severity) {
    ...
    case "medium":
      return "border-amber-400/35 bg-amber-500/12 text-amber-800 dark:text-amber-100"
    case "high":
      return "border-orange-400/35 bg-orange-500/12 text-orange-800 dark:text-orange-100"
    default:
      return "border-red-400/35 bg-red-500/12 text-red-800 dark:text-red-100"
  }
}
...
function RejectionCategoryChip({ category }: { category: MarketplacePayoutRejectionCategory }) {
  return (
    <Tooltip>
      ...
      <TooltipContent className="max-w-xs border-border bg-background text-gray-100">
  ...
  return (
    <TooltipProvider delayDuration={150}>
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        ...
        <p className="text-sm text-muted-foreground">{filteredItems.length} of {items.length} pending review</p>
      </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto]">
        <div className="rounded-2xl border border-border bg-background/80 p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
            <div className="grid flex-1 gap-3 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="queue-filter-category" className="text-sm text-muted-foreground">Filter by latest rejection category</Label>
                <select
              ...
              <div className="space-y-2">
                <Label htmlFor="queue-filter-severity" className="text-sm text-muted-foreground">Filter by latest blocker severity</Label>
                <select
            ...
            <div className="flex flex-wrap gap-2 lg:max-w-[320px] lg:justify-end">
              <Button
                variant="outline"
                className={`border-border bg-background text-gray-200 hover:bg-gray-800 ${categoryFilter === "compliance" ? "border-violet-400/50 bg-violet-500/10 text-violet-800 dark:text-violet-100" : ""}`}
              ...
              <Button
                variant="outline"
                className={`border-border bg-background text-gray-200 hover:bg-gray-800 ${severityFilter === "critical" ? "border-red-400/50 bg-red-500/10 text-red-800 dark:text-red-100" : ""}`}
              ...
              <Button
                variant="outline"
                className="border-cyan-400/35 bg-cyan-500/10 text-cyan-800 dark:text-cyan-100 hover:bg-cyan-500/20"
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button
              variant="outline"
              className={`border-border bg-background text-gray-200 hover:bg-gray-800 ${categoryFilter === "compliance" && severityFilter === "all" ? "border-violet-400/50 bg-violet-500/10 text-violet-800 dark:text-violet-100" : ""}`}
            ...
            <Button
              variant="outline"
              className={`border-border bg-background text-gray-200 hover:bg-gray-800 ${categoryFilter === "all" && severityFilter === "critical" ? "border-red-400/50 bg-red-500/10 text-red-800 dark:text-red-100" : ""}`}
            ...
            <Button
              variant="outline"
              className={`border-border bg-background text-gray-200 hover:bg-gray-800 ${categoryFilter === "compliance" && severityFilter === "critical" ? "border-rose-400/50 bg-rose-500/10 text-rose-800 dark:text-rose-100" : ""}`}
              onClick={() => applyQuickFilter("compliance", "critical")}
            >
              Incident mode
            </Button>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">Queue filters use the latest persisted payout-review metadata on each transaction and persist in the URL with compact aliases for reproducible review views.</p>
        </div>

        <div className="rounded-2xl border border-border bg-background/80 p-4">
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">Chip Guide</p>
          <div className="mt-3 flex flex-wrap gap-2">
          ...
          <p className="mt-3 text-xs text-muted-foreground">Hover any chip or badge for review guidance.</p>
        </div>
      </div>

      {error ? <p className="mt-4 rounded-xl border border-red-800 bg-red-950 p-3 text-sm text-red-700 dark:text-red-300">{error}</p> : null}
      {isPending ? <p className="mt-4 text-sm text-muted-foreground">Refreshing...</p> : null}
      ...
      {filteredItems.length ? (
            ...
            return (
              <div key={item.id} className="rounded-2xl border border-border bg-background p-5">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-rose-700 dark:text-rose-300">{item.status.toLowerCase()} review queue</p>
                    <h3 className="mt-2 text-lg font-semibold text-foreground">{item.discoveryTitle}</h3>
                    <p className="mt-2 text-sm text-muted-foreground">Scientist: {item.scientistName} · Sponsor: {item.sponsorName}</p>
                    <p className="mt-1 text-sm text-muted-foreground">Submitted {new Date(item.updatedAt).toLocaleString()} · Deal room {item.dealRoomId}</p>
                    {persistedReview ? (
                      <div className="mt-3">
                        <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Latest stored rejection</p>
                      ...
                      <p className="mt-3 text-xs text-muted-foreground">No prior rejection metadata recorded for this payout.</p>
                    )}
                  </div>
                  ...
                  <div className="min-w-[220px] rounded-2xl border border-border bg-background p-4">
                    ...
                    <p className="mt-1 text-2xl font-semibold text-foreground">{formatCurrency(item.amountCents, item.currency)}</p>
                    <p className="mt-2 text-sm text-muted-foreground">Net payout {formatCurrency(item.payoutCents, item.currency)}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <Button variant="outline" className="border-red-400/40 bg-red-500/10 text-red-800 dark:text-red-100 hover:bg-red-500/20" disabled={isBusy} onClick={() => void handleAction(item, "rejectPayoutReview")}>Reject review</Button>
                ...
                <div className="mt-4 space-y-2 rounded-2xl border border-border bg-background p-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor={`rejection-category-${item.id}`} className="text-sm text-muted-foreground">Category</Label>
                      <select
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`rejection-severity-${item.id}`} className="text-sm text-muted-foreground">Blocker severity</Label>
                      <select
                  ...
                  <Label htmlFor={`rejection-note-${item.id}`} className="text-sm text-muted-foreground">Rejection summary</Label>
                  <Textarea
                    ...
                    disabled={isBusy}
                  />
                  <Label htmlFor={`follow-up-action-${item.id}`} className="text-sm text-muted-foreground">Required follow-up action</Label>
                  <Textarea
                    ...
                    disabled={isBusy}
                  />
                  <p className="text-xs text-muted-foreground">These fields are stored in transaction metadata and rendered separately back to the scientist in the deal room when review is rejected.</p>
                </div>
              </div>
            )
          })}
        ...
        <p className="mt-6 text-sm text-muted-foreground">No pending payouts match the current rejection category and severity filters.</p>
      ) : (
        <p className="mt-6 text-sm text-muted-foreground">No marketplace payouts are waiting for admin review.</p>
      )}
      </section>
    </TooltipProvider>
  )
