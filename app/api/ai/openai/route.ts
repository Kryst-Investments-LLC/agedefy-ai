...
export const POST = withHttpMetrics('/api/ai/openai', async (request: NextRequest) => {
  ...
  try {
    ...
    const requestedCredits = estimateAICreditCost({ operation: 'openai-query', maxResults })
    ...
    logRequestEvent('info', 'Governed OpenAI request started', requestContext, {
    ...
    return await executeRouteIdempotentJsonMutation({
      ...
      tenantId: requestContext.tenantId,
      actorUserId: session.user.id,
      requestFingerprint: createIdempotencyFingerprint({ provider: 'openai', userId: session.user.id, payload: parsedPayload.data }),
      execute: async () => {
        return runWithReservedAICredits({
          ...
          provider: 'openai',
          model: config.providers.openai.model,
          description: 'OpenAI governed query',
          metadata: {
            requestId: requestContext.requestId,
            maxResults,
          },
          ...
          execute: async () => {
            ...
            aiRequestCounter.add(1, { provider: 'openai', model: config.providers.openai.model })

            return withSpan('ai.provider.openai', { 'ai.provider': 'openai', 'ai.model': config.providers.openai.model }, async () => {
              const response = await executeWithCircuitBreaker({
                ...
                execute: async () => {
                  ...
                  if (!providerResponse.ok && (providerResponse.status === 429 || providerResponse.status >= 500)) {
                    if (providerResponse.status === 429) aiProviderQuotaCounter.add(1, { provider: 'openai' })
                    throw new Error(`OpenAI upstream request failed with status ${providerResponse.status}`)
                  }
              ...
              if (!response.ok) {
                ...
                logRequestEvent('error', 'OpenAI API error', requestContext, { status: response.status, error })
                return { status: response.status, body: { error: 'OpenAI API request failed' } }
              }
              ...
              aiRequestLatencyHistogram.record(Date.now() - startTime, { provider: 'openai' })
              aiRequestCostHistogram.record(cost, { provider: 'openai' })
    ...
    if (error instanceof CircuitBreakerOpenError) {
      logRequestEvent('warn', 'OpenAI circuit breaker open', requestContext, {
        dependency: error.dependency,
        retryAt: error.retryAt?.toISOString(),
      })
      return withRequestContextHeaders(NextResponse.json(
        { error: 'OpenAI is temporarily unavailable. Please retry shortly.' },
        {
          status: 503,
    ...
    if (error instanceof AIGovernanceError) {
      await enqueueGovernedAIAuditJob({
        ...
        payload: {
          provider: 'openai',
          model: getAIConfig().providers.openai.model,
          route: requestContext.path,
          requestId: requestContext.requestId,
          queryLength: 0,
          tenantId: requestContext.tenantId,
    ...
    logRequestEvent('error', 'OpenAI route error', requestContext, {
