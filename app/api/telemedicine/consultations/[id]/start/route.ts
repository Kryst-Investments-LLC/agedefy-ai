...
import { consultationStartSchema } from "@/lib/validators/telemedicine"
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const parsed = consultationStartSchema.safeParse(body)
  if (!parsed.success) {
  ...
  if (consultation.status !== "SCHEDULED") {
    return NextResponse.json({ error: `Cannot transition consultation from ${consultation.status} to IN_PROGRESS` }, { status: 409 })
  }
  ...
  return executeRouteIdempotentJsonMutation({
    ...
    requestFingerprint: createIdempotencyFingerprint({ userId: authResult.user.id, consultationId: id, payload: parsed.data, action: "start" }),
    execute: async () => {
      ...
      await logAudit({
        ...
        tenantId: tenantContext.tenantId,
        action: "consultation.in_progress",
        entityType: "ConsultationRequest",
        entityId: id,
      })

      logger.info("Consultation started", { consultationId: id, actor: authResult.user.id })
