      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="mb-6 flex gap-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-9 w-24 animate-pulse rounded-lg bg-card" />
          ))}
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-border bg-background p-5">
              ...
              <div className="mt-3 h-5 w-40 animate-pulse rounded bg-card" />
              <div className="mt-2 h-3 w-full animate-pulse rounded bg-card" />
              <div className="mt-4 flex items-center justify-between">
                <div className="h-7 w-16 animate-pulse rounded bg-card" />
