            ...
            {categories.map((category) => {
              ...
              return (
                <section key={category}>
                  <h2 className="text-lg font-semibold text-teal-400 mb-4 capitalize">
                    {category}
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {group.map((pathway) => (
                      <Link
                        ...
                        className="block bg-gray-800/50 border border-gray-700 rounded-lg p-5 hover:border-teal-600 transition-colors"
                      >
                        <h3 className="text-white font-medium mb-1">{pathway.name}</h3>
                        {pathway.description && (
