...
async function handleReminderNotifyDue(job: OrchestrationJob) {
  logger.info("Reminder delivery sweep started", { jobId: job.id })
  const result = await sweepDueReminders()
  logger.info("Reminder delivery sweep complete", { ...result })
  return result
}
...
export async function processOrchestrationJob(job: OrchestrationJob) {
  ...
  switch (job.jobType) {
    ...
    case "reminder.notify-due":
      return handleReminderNotifyDue(job)
    default:
